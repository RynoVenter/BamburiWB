...
public class DelTruckReject extends VtiUserExit
{
	public VtiUserExitResult execute() throws VtiExitException
	{
		...
		VtiExitLdbTable icHeaderTWRejLdbTable = getLocalDatabaseTable("YSWB_IC_HEADER");
		...
		if (icHeaderTWRejLdbTable == null) return new VtiUserExitResult(999, "Unable to initialise table YSWB_IC_HEADER.");
		...
		sessionHeader.setNextFunctionId("YSWB_TRIC");
		
		//Dataset Declaration
		VtiExitLdbSelectCriterion [] icHeaderTWRejSelConds = 
		{
		...
		VtiExitLdbSelectConditionGroup icHeaderTWRejSelCondGrp = new VtiExitLdbSelectConditionGroup(icHeaderTWRejSelConds, true);
		VtiExitLdbTableRow[] icHeaderTWRejLdbRows = icHeaderTWRejLdbTable.getMatchingRows(icHeaderTWRejSelCondGrp);

		if(icHeaderTWRejLdbRows.length == 0)
		...
		String icStatus = statusRejLdbRows[0].getFieldValue("WGH_STATUS");
		
		if(icStatus.equalsIgnoreCase("Failed") || icStatus.equalsIgnoreCase("Complete") 
				||icStatus.equalsIgnoreCase("New")  || icStatus.substring(0,1).equalsIgnoreCase("0"))
		...
		if(wbTWRejLdbRows.length == 0)
		{
			...
			ldbRowWeigh1.setFieldValue("VTIREFA", icHeaderTWRejLdbRows[0].getFieldValue("VTIREF"));
		...
		catch (VtiExitException ee)
		{
			...
			if(qTLdbRows.length == 0)
				return new VtiUserExitResult(000,"Rejection not possible. Process already rejected or complete.");
