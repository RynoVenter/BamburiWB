...
public class SavePackingList extends VtiUserExit 
{
	public VtiUserExitResult execute() throws VtiExitException 
	{
		...
		if(!scrFRefDoc.getFieldValue().startsWith("R"))
		{
			sessionHeader.setNextFunctionId("YSWB_PACKED");
			...
			catch(VtiExitException ee)
			{
				...
				if(scrFShift.getFieldValue().length() < 1)
					return new VtiUserExitResult(999,1,"Please select the Shift.");
				
				if(sTblRowCount < 1)
					return new VtiUserExitResult(999,1,"Please select the line packed.");
				
				if(scrFTruckReg.getFieldValue().length() < 1)
					return new VtiUserExitResult(999,1,"Please select the Registration Number.");
			...
			for(int sTblRowInc = 0; sTblRowInc < sTblRowCount; sTblRowInc++)
			{
				...
				if(currRow.getFieldValue("QUEUE").length() < 1)
					return new VtiUserExitResult(999,"Packing has not been completed for this truck, please reconfirm.");
			    ...
			    {	
					...
					VtiExitLdbSelectCriterion [] wbSelConds = 
					{
						new VtiExitLdbSelectCondition("SERVERGROUP", VtiExitLdbSelectCondition.EQ_OPERATOR, getServerGroup()),
							new VtiExitLdbSelectCondition("SERVERID", VtiExitLdbSelectCondition.EQ_OPERATOR, getServerId()),
								new VtiExitLdbSelectCondition(statusOrd, VtiExitLdbSelectCondition.EQ_OPERATOR, scrFRefDoc.getFieldValue()),
									new VtiExitLdbSelectCondition("TRUCKREG", VtiExitLdbSelectCondition.EQ_OPERATOR, scrFTruckReg.getFieldValue()),
						new VtiExitLdbSelectCondition("DEL_IND", VtiExitLdbSelectCondition.NE_OPERATOR, "X")
					};
				
					VtiExitLdbSelectConditionGroup wbSelCondGrp = new VtiExitLdbSelectConditionGroup(wbSelConds, true);
					VtiExitLdbTableRow[] wbLdbRows = ldbWb.getMatchingRows(wbSelCondGrp);
					
					if(wbLdbRows.length == 0)
						return new VtiUserExitResult(99, "No matching WB records.");
					else
						if(wbLdbRows[0].getLongFieldValue("WEIGHT1") < 1 && wbLdbRows.length == 1)
						{
							return new VtiUserExitResult(999, "Weigh 1 was rejected without a weight recorded.");
						}
					...
					ldbRowPacking.setFieldValue("STATUS", "PACKED");
					...
					if(!scrFRefDoc.getFieldValue().startsWith("0"))
						ldbRowPacking.setFieldValue("OFF_ORDER", scrFRefDoc.getFieldValue());
		...
		sessionHeader.setNextFunctionId("YSWB_PACKED_RSO");
			...
			if(scrFTruckReg.getFieldValue().length() < 1)
				return new VtiUserExitResult(999,"Please select the Registration Number.");
		...
		for(int sTblRowInc = 0; sTblRowInc < sTblRowCount; sTblRowInc++)
		{
		    ...
		    {	
				...
				if(scrDocType.getFieldValue().equalsIgnoreCase(""))
				{
					ldbRowPacking.setFieldValue("VBELN", currRow.getFieldValue("VBELN_IP"));
					tranType = "PACKING";
					orderDoc = "VBELN";
					statusOrd = "VBELN";
				}
