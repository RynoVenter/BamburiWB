...
public class POSync extends VtiUserExit 
{
	public VtiUserExitResult execute() throws VtiExitException 
	{
		...
		VtiUserExitScreenField scrTruck = getScreenField("REGNO");
		
		if (scrOrder == null) return new VtiUserExitResult(999, "Unable to initialise field EBELN.");
		if (scrRef == null) return new VtiUserExitResult(999, "Unable to initialise field VTIREF.");
		if (scrTruck == null) return new VtiUserExitResult(999, "Unable to initialise field REGNO.");
		...
		VtiExitLdbSelectCriterion [] statSelConds = 
		{
				new VtiExitLdbSelectCondition("SERVERGRP", VtiExitLdbSelectCondition.EQ_OPERATOR, getServerGroup()),
					new VtiExitLdbSelectCondition("SERVERID", VtiExitLdbSelectCondition.EQ_OPERATOR, getServerId()),
						new VtiExitLdbSelectCondition("EBELN", VtiExitLdbSelectCondition.EQ_OPERATOR, scrOrder.getFieldValue()),
		...
		if(statLdbRows.length == 0)
			return new VtiUserExitResult(999, "No purchase order matching the screen criteria match the Status table. Requires admin correction.");
		...
		if(regGetStatus.equalsIgnoreCase("W") && wbLdbRows[0].getFieldValue("WEIGHT2").length() > 0)
			regStatus = "WEIGH 2";
		...
		sessionInfo.setNextFunctionId("YSWB_TRINBOUND");
