...
public class WeighbridgeFormat extends VtiUserExit
{
	...
	//If status from sales order is new or changed or rejected then it's 1 weight, else set to weight 2
		...
		if(!isBulk)
		{
			...
			VtiExitLdbTableRow[] loadLdbRows = loadingLdbTable.getMatchingRows(loadingSelCondGrp);
		
			if(packingTLdbRows.length != loadLdbRows.length)
			return new VtiUserExitResult(999,1,"This truck did (" + packingTLdbRows.length + ") packings and (" + loadLdbRows.length + ") loadings, please ensure they are equal before processing weigh.");*/
			if(pRecs == 0)
			{
				packerLD = ""; 
				shiftLD = ""; 
		...
		if(wbRegisterLdbRows.length > 0)
		{
		 ...
		 wghbr.setFieldValue("FIELDVALUE","");
