...
public class GetBridgeWeight2 extends VtiUserExit
{
	...
	{	
		...
		if(scrFDispWeight.getDoubleFieldValue() == 0)
		{
			...
			Log.trace(2, "weight before writting to screenfield is " + stWeightReading);
			try
			{
				if(stWeightReading.length() > 0)
				{
					scrFDispWeight.setFieldValue(stWeightReading);
					Log.trace(2, "weight writting to screenfield is " + stWeightReading);
				}
				else
				{
					
					Log.trace(2, "weight before writting to screenfield is " + 0);
					scrFDispWeight.setFieldValue("0");
				}
			}
			catch( NullPointerException npe)
